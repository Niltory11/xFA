...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_receipt'])) {
    ...
    // Debug logging
    error_log("Receipt validation - entry_date: " . $entry_date);
    error_log("Receipt validation - type: " . $type);
    error_log("Receipt validation - asset_account_id: " . $asset_account_id);
    error_log("Receipt validation - particular_account_id: " . $particular_account_id);
    error_log("Receipt validation - amount: " . $amount);
    ...
    if (
        ...
        error_log("Receipt validation failed - missing fields: " . implode(', ', $missing_fields));
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        if (receiptType.value === 'bank') {
            ...
            if (!bankSelect.value) {
                e.preventDefault();
                showAlert('danger', 'Please select a bank account when receipt type is Bank.');
                bankSelect.focus();
                return false;
            }
        }
